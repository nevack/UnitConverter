...
- Gradle projects are `:app`, `:core:converter`, `:core:design`, `:feature:converter`, `:feature:history-api`, `:feature:history`, and `:nbrb-api`. `build-logic` is an included build, not a normal subproject.
...
## Architecture
- The app is Compose-based inside AppCompat activities. `:app`, `:core:design`, `:feature:converter`, and `:feature:history` enable Compose.
- Navigation is manual AppCompat activity wiring; there is no Navigation Component graph.
- Launcher entrypoint is `app/src/main/kotlin/dev/nevack/unitconverter/categories/CategoriesActivity.kt`.
- `CategoriesActivity` handles the main category list. On tablets it renders the converter pane with `converterRoute`; on phones it launches `ConverterActivity`.
...
- `:core:design` owns shared Compose theme/color definitions.
...
- Spotless coverage is split: root `spotlessCheck` formats `*.gradle.kts`, while modules using the `dev.nevack.plugins.module-quality` convention format Kotlin source. `build-logic` Kotlin sources are outside those module targets.
